...
struct Emitter<T> {
  ...
  fn on(self, handler: (T) -> Void) -> (Int, Emitter<T>) {
  ...
  fn once(self, handler: (T) -> Void) -> (Int, Emitter<T>) {
  ...
  fn off(self, listenerId: Int) -> Emitter<T> {
  ...
  fn emit(self, value: T) -> Emitter<T> {
  ...
  fn listenerCount(self) -> Int => self.listeners.length

  fn removeAll(self) -> Emitter<T> {
...
struct EventBus {
  ...
  fn on(self, channel: String, handler: (String) -> Void) -> EventBus {
    let emitter = self._getOrCreate(channel)
    let {_, updated} = emitter.on(handler)
    return self._setChannel(channel, updated)
  }

  fn emit(self, channel: String, value: String) -> EventBus {
    var i = 0
    while i < self.channels.length {
      let {name, emitter} = self.channels[i]
      if name == channel {
        let updated = emitter.emit(value)
        return self._setChannel(channel, updated)
      }
      i += 1
    }
  ...
  fn _getOrCreate(self, channel: String) -> Emitter<String> {
    for entry in self.channels {
      let {name, emitter} = entry
      if name == channel { return emitter }
    }
    return Emitter.new()
  }

  fn _setChannel(self, channel: String, emitter: Emitter<String>) -> EventBus {
